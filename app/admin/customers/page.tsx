...
export default async function CustomersPage({ searchParams }: { searchParams: Promise<{ q?: string }> }) {
  ...
  return <div className="mx-auto max-w-5xl"><div className="mb-7"><p className="text-sm font-semibold text-brand-600">Customer records</p><h1 className="text-3xl font-black">Search customers</h1></div>
    <form className="card mb-6 flex gap-2 p-3"><SearchAutocomplete name="q" defaultValue={q} placeholder="Name, phone number, or repair number" scope="customers" submitOnSelect /><button className="btn-primary"><Search size={16} />Search</button></form>
